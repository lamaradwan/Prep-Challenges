...
const classesAvg = (data) => {
    ...
    for(let g =0; g < grades.length; g++){
        for(let c=0; c < grades[g].classes.length; c++){
            sum =0;
            average =0;
            for(let cs=0; cs < grades[g].classes[c].classScores.length; cs++){
                sum += grades[g].classes[c].classScores[cs];
                average = sum / grades[g].classes[c].classScores.length;

            }
            grades[g].classes[c].avg = Math.floor(average);
        }
    }
    return data;
};
