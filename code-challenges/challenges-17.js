...
const recursionPattern = (int1, int2) => {
    ...
    var newNumber = int1 - int2;

    // base case
    if (newNumber > 0) {
        recursionPattern(newNumber,int2);
    }
    
    if (newNumber < 0) {
            var newNumber = int1 - int2;
            arr.push(newNumber)
            var newNumber = newNumber + int2;
    }
    arr.push(int1)
    return(arr)
    //another way
...
const filterLinks = (str) => {
    // write your code here
    return str.match(/www(.*?).(.com|.org|.net)/i)[0]
}
// -------------------------------------------------------------------------------------------------------
...
const isPalindrome = (str) => {
    ...
    while(stringToArr.includes(' ')){
        let index = stringToArr.indexOf(' ');
    ...
    let reversedString = stringToArr.slice().reverse().join('');
    let newString = stringToArr.join('');
    if(reversedString.toLocaleLowerCase() == newString.toLocaleLowerCase()){
        return true;
    }
    else{
        return false;
